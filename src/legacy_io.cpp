...
inline int32_t swap_int32(int32_t i) { return static_cast<int32_t>(bswap32(static_cast<uint32_t>(i))); }

inline int16_t swap_int16(int16_t val) { return static_cast<int16_t>(bswap16(static_cast<uint16_t>(val))); }

inline int64_t swap_int64(int64_t val) { return static_cast<int64_t>(bswap64(static_cast<uint64_t>(val))); }
...
bool load_trk(const std::string &filename, Tractogram &tr) {
  ...
  if (std::string(header->magic_number, 5) != "TRACK")
    return false;
  ...
  tr.header = json11::Json::object{
      ...
      {"VOXEL_TO_RASMM",
       json11::Json::array{json11::Json::array{header->voxel_to_rasmm[0][0],
                                               header->voxel_to_rasmm[0][1],
                                               header->voxel_to_rasmm[0][2],
                                               header->voxel_to_rasmm[0][3]},
                           json11::Json::array{header->voxel_to_rasmm[1][0],
                                               header->voxel_to_rasmm[1][1],
                                               header->voxel_to_rasmm[1][2],
                                               header->voxel_to_rasmm[1][3]},
                           json11::Json::array{header->voxel_to_rasmm[2][0],
                                               header->voxel_to_rasmm[2][1],
                                               header->voxel_to_rasmm[2][2],
                                               header->voxel_to_rasmm[2][3]},
                           json11::Json::array{header->voxel_to_rasmm[3][0],
                                               header->voxel_to_rasmm[3][1],
                                               header->voxel_to_rasmm[3][2],
                                               header->voxel_to_rasmm[3][3]}}}};
  ...
  while (offset + sizeof(int32_t) <= buffer.size()) {
    ...
    if (bytes_needed > buffer.size() - offset)
      return false;
    ...
    for (int32_t j = 0; j < n_points; ++j) {
      ...
      float x = cx * header->voxel_to_rasmm[0][0] + cy * header->voxel_to_rasmm[0][1] +
                cz * header->voxel_to_rasmm[0][2] + header->voxel_to_rasmm[0][3];
      float y = cx * header->voxel_to_rasmm[1][0] + cy * header->voxel_to_rasmm[1][1] +
                cz * header->voxel_to_rasmm[1][2] + header->voxel_to_rasmm[1][3];
      float z = cx * header->voxel_to_rasmm[2][0] + cy * header->voxel_to_rasmm[2][1] +
                cz * header->voxel_to_rasmm[2][2] + header->voxel_to_rasmm[2][3];
...
bool load_tck(const std::string &filename, Tractogram &tr) {
  ...
  if (offset_end == std::string_view::npos || offset_end == offset_pos)
    return false;
  size_t offset;
  try {
...
bool load_vtk(const std::string &filename, Tractogram &tr) {
  try {
    ...
    if (num_points > std::numeric_limits<size_t>::max() / (3 * elem_size))
    ...
    f.seekg(data_start);
    if (num_points * 3 * elem_size > bytes_available)
      return false;
    ...
    if (has_offsets) {
      ...
      const std::streampos offsets_start = f.tellg();
      f.seekg(0, std::ios::end);
      const size_t off_bytes_avail = static_cast<size_t>(f.tellg() - offsets_start);
      f.seekg(offsets_start);
      const size_t offset_elem_size = is_int64 ? sizeof(uint64_t) : sizeof(uint32_t);
      if (num_offsets > off_bytes_avail / offset_elem_size)
        return false;
    ...
    const std::streampos lines_start = f.tellg();
    f.seekg(0, std::ios::end);
    const size_t lines_bytes_avail = static_cast<size_t>(f.tellg() - lines_start);
    f.seekg(lines_start);
    ...
    for (size_t i = 0; i < num_streamlines; ++i) {
      ...
      if (n_pts == 0)
        continue;
      if (static_cast<size_t>(n_pts) * sizeof(int32_t) > lines_bytes_avail)
        return false;
    ...
    if (tr.offsets.empty() || tr.offsets.size() != num_streamlines + 1)
      return false;
...
bool load_nifti_header(const std::string &ref_path, json11::Json &out_header) {
  ...
  if (sizeof_hdr == 348) { // NIfTI-1
    ...
    if (swap_endian) {
      qform16 = swap_int16(qform16);
      sform16 = swap_int16(sform16);
    }
    qform_code = qform16;
    sform_code = sform16;
    ...
    if (swap_endian) {
      ...
      c = swap_float(c);
      d = swap_float(d);
      qoffset_x = swap_float(qoffset_x);
      qoffset_y = swap_float(qoffset_y);
      qoffset_z = swap_float(qoffset_z);
    }
    ...
    if (swap_endian) {
      qform32 = swap_int32(qform32);
      sform32 = swap_int32(sform32);
    }
    qform_code = qform32;
    sform_code = sform32;
    ...
    c = static_cast<float>(qc);
    d = static_cast<float>(qd);
    qoffset_x = static_cast<float>(qox);
    qoffset_y = static_cast<float>(qoy);
    qoffset_z = static_cast<float>(qoz);
  ...
  if (sform_code > 0) {
    ...
    v2r[0][0] = R[0][0] * dx;
    v2r[0][1] = R[0][1] * dy;
    v2r[0][2] = R[0][2] * qfac * dz;
    v2r[0][3] = qoffset_x;
    v2r[1][0] = R[1][0] * dx;
    v2r[1][1] = R[1][1] * dy;
    v2r[1][2] = R[1][2] * qfac * dz;
    v2r[1][3] = qoffset_y;
    v2r[2][0] = R[2][0] * dx;
    v2r[2][1] = R[2][1] * dy;
    v2r[2][2] = R[2][2] * qfac * dz;
    v2r[2][3] = qoffset_z;
    v2r[3][0] = 0;
    v2r[3][1] = 0;
    v2r[3][2] = 0;
  ...
  out_header =
      json11::Json::object{{"DIMENSIONS", json11::Json::array{dims[0], dims[1], dims[2]}},
                           {"VOXEL_TO_RASMM",
                            json11::Json::array{json11::Json::array{v2r[0][0], v2r[0][1], v2r[0][2], v2r[0][3]},
...
bool save_trx(const Tractogram &tr, const std::string &out_path, const std::string &ref_nifti_path) {
  try {
    ...
    if (header_to_use.is_null() || !header_to_use["VOXEL_TO_RASMM"].is_array() ||
        header_to_use["VOXEL_TO_RASMM"].array_items().empty()) {
      if (ref_nifti_path.empty()) {
        std::cerr << "Error: TCK/VTK -> TRX requires a reference NIfTI file\n";
        return false;
      }
      json11::Json ref_hdr;
      if (!load_nifti_header(ref_nifti_path, ref_hdr))
        return false;
    ...
    if (tr.offsets.empty())
      return false; // offsets must hold at least the trailing sentinel
    ...
    auto header_obj = header_to_use.is_object() ? header_to_use.object_items() : std::map<std::string, json11::Json>();
...
bool invert_matrix4x4(const float m[4][4], float invOut[4][4]) {
  ...
  inv[0] = m_1d[5] * m_1d[10] * m_1d[15] - m_1d[5] * m_1d[11] * m_1d[14] - m_1d[9] * m_1d[6] * m_1d[15] +
           m_1d[9] * m_1d[7] * m_1d[14] + m_1d[13] * m_1d[6] * m_1d[11] - m_1d[13] * m_1d[7] * m_1d[10];
  inv[4] = -m_1d[4] * m_1d[10] * m_1d[15] + m_1d[4] * m_1d[11] * m_1d[14] + m_1d[8] * m_1d[6] * m_1d[15] -
           m_1d[8] * m_1d[7] * m_1d[14] - m_1d[12] * m_1d[6] * m_1d[11] + m_1d[12] * m_1d[7] * m_1d[10];
  inv[8] = m_1d[4] * m_1d[9] * m_1d[15] - m_1d[4] * m_1d[11] * m_1d[13] - m_1d[8] * m_1d[5] * m_1d[15] +
           m_1d[8] * m_1d[7] * m_1d[13] + m_1d[12] * m_1d[5] * m_1d[11] - m_1d[12] * m_1d[7] * m_1d[9];
  inv[12] = -m_1d[4] * m_1d[9] * m_1d[14] + m_1d[4] * m_1d[10] * m_1d[13] + m_1d[8] * m_1d[5] * m_1d[14] -
            m_1d[8] * m_1d[6] * m_1d[13] - m_1d[12] * m_1d[5] * m_1d[10] + m_1d[12] * m_1d[6] * m_1d[9];
  inv[1] = -m_1d[1] * m_1d[10] * m_1d[15] + m_1d[1] * m_1d[11] * m_1d[14] + m_1d[9] * m_1d[2] * m_1d[15] -
           m_1d[9] * m_1d[3] * m_1d[14] - m_1d[13] * m_1d[2] * m_1d[11] + m_1d[13] * m_1d[3] * m_1d[10];
  inv[5] = m_1d[0] * m_1d[10] * m_1d[15] - m_1d[0] * m_1d[11] * m_1d[14] - m_1d[8] * m_1d[2] * m_1d[15] +
           m_1d[8] * m_1d[3] * m_1d[14] + m_1d[12] * m_1d[2] * m_1d[11] - m_1d[12] * m_1d[3] * m_1d[10];
  inv[9] = -m_1d[0] * m_1d[9] * m_1d[15] + m_1d[0] * m_1d[11] * m_1d[13] + m_1d[8] * m_1d[1] * m_1d[15] -
           m_1d[8] * m_1d[3] * m_1d[13] - m_1d[12] * m_1d[1] * m_1d[11] + m_1d[12] * m_1d[3] * m_1d[9];
  inv[13] = m_1d[0] * m_1d[9] * m_1d[14] - m_1d[0] * m_1d[10] * m_1d[13] - m_1d[8] * m_1d[1] * m_1d[14] +
            m_1d[8] * m_1d[2] * m_1d[13] + m_1d[12] * m_1d[1] * m_1d[10] - m_1d[12] * m_1d[2] * m_1d[9];
  inv[2] = m_1d[1] * m_1d[6] * m_1d[15] - m_1d[1] * m_1d[7] * m_1d[14] - m_1d[5] * m_1d[2] * m_1d[15] +
           m_1d[5] * m_1d[3] * m_1d[14] + m_1d[13] * m_1d[2] * m_1d[7] - m_1d[13] * m_1d[3] * m_1d[6];
  inv[6] = -m_1d[0] * m_1d[6] * m_1d[15] + m_1d[0] * m_1d[7] * m_1d[14] + m_1d[4] * m_1d[2] * m_1d[15] -
           m_1d[4] * m_1d[3] * m_1d[14] - m_1d[12] * m_1d[2] * m_1d[7] + m_1d[12] * m_1d[3] * m_1d[6];
  inv[10] = m_1d[0] * m_1d[5] * m_1d[15] - m_1d[0] * m_1d[7] * m_1d[13] - m_1d[4] * m_1d[1] * m_1d[15] +
            m_1d[4] * m_1d[3] * m_1d[13] + m_1d[12] * m_1d[1] * m_1d[7] - m_1d[12] * m_1d[3] * m_1d[5];
  inv[14] = -m_1d[0] * m_1d[5] * m_1d[14] + m_1d[0] * m_1d[6] * m_1d[13] + m_1d[4] * m_1d[1] * m_1d[14] -
            m_1d[4] * m_1d[2] * m_1d[13] - m_1d[12] * m_1d[1] * m_1d[6] + m_1d[12] * m_1d[2] * m_1d[5];
  inv[3] = -m_1d[1] * m_1d[6] * m_1d[11] + m_1d[1] * m_1d[7] * m_1d[10] + m_1d[5] * m_1d[2] * m_1d[11] -
           m_1d[5] * m_1d[3] * m_1d[10] - m_1d[9] * m_1d[2] * m_1d[7] + m_1d[9] * m_1d[3] * m_1d[6];
  inv[7] = m_1d[0] * m_1d[6] * m_1d[11] - m_1d[0] * m_1d[7] * m_1d[10] - m_1d[4] * m_1d[2] * m_1d[11] +
           m_1d[4] * m_1d[3] * m_1d[10] + m_1d[8] * m_1d[2] * m_1d[7] - m_1d[8] * m_1d[3] * m_1d[6];
  inv[11] = -m_1d[0] * m_1d[5] * m_1d[11] + m_1d[0] * m_1d[7] * m_1d[9] + m_1d[4] * m_1d[1] * m_1d[11] -
            m_1d[4] * m_1d[3] * m_1d[9] - m_1d[8] * m_1d[1] * m_1d[7] + m_1d[8] * m_1d[3] * m_1d[5];
  inv[15] = m_1d[0] * m_1d[5] * m_1d[10] - m_1d[0] * m_1d[6] * m_1d[9] - m_1d[4] * m_1d[1] * m_1d[10] +
            m_1d[4] * m_1d[2] * m_1d[9] + m_1d[8] * m_1d[1] * m_1d[6] - m_1d[8] * m_1d[2] * m_1d[5];
...
static std::array<char, 3> axcodes_from_affine(const float aff[4][4]) {
  ...
  for (int col = 0; col < 3; ++col) {
    float norm = std::sqrt(aff[0][col] * aff[0][col] + aff[1][col] * aff[1][col] + aff[2][col] * aff[2][col]);
    if (norm == 0.f)
      norm = 1.f;
...
bool save_trk(const Tractogram &tr,
              const std::string &out_path,
              const std::string &original_filename,
              const std::string &ref_nifti_path) {
  ...
  if (header_to_use["VOXEL_TO_RASMM"].is_array()) {
    ...
    if (rows.size() >= 4) {
      ...
      header.voxel_sizes[0] = std::sqrt(vox_to_ras[0][0] * vox_to_ras[0][0] + vox_to_ras[1][0] * vox_to_ras[1][0] +
                                        vox_to_ras[2][0] * vox_to_ras[2][0]);
      header.voxel_sizes[1] = std::sqrt(vox_to_ras[0][1] * vox_to_ras[0][1] + vox_to_ras[1][1] * vox_to_ras[1][1] +
                                        vox_to_ras[2][1] * vox_to_ras[2][1]);
      header.voxel_sizes[2] = std::sqrt(vox_to_ras[0][2] * vox_to_ras[0][2] + vox_to_ras[1][2] * vox_to_ras[1][2] +
                                        vox_to_ras[2][2] * vox_to_ras[2][2]);
    }
  }
