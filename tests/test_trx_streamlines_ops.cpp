...
namespace
{
	Streamline make_streamline_ones()
	{
		return Streamline::Ones(30, 3);
	}
...
TEST_P(StreamlinesOpsIntersectionTest, Intersection)
{
	...
	auto result = trxmmap::perform_streamlines_operation(trxmmap::intersection,
	                                                     {streamlines_new, streamlines_ori},
	                                                     precision);
...
TEST_P(StreamlinesOpsUnionTest, Union)
{
	...
	auto result = trxmmap::perform_streamlines_operation(trxmmap::union_maps,
	                                                     {streamlines_new, streamlines_ori},
	                                                     precision);
...
TEST_P(StreamlinesOpsDifferenceTest, Difference)
{
	...
	auto result = trxmmap::perform_streamlines_operation(trxmmap::difference,
	                                                     {streamlines_new, streamlines_ori},
	                                                     precision);
	EXPECT_EQ(result.first.size(), expected);
}
